...
This page describes a demo for a reproductive L\+S\+PG R\+OM applied to a 1D advection-\/diffusion problem using a nonlinear manifold via kernel P\+CA. ~\newline
 To jump directly at the full demo script, click \href{https://github.com/Pressio/pressio4py/blob/master/demos/unsteady_default_lspg_advdiff1d_kpca/main.py}{\texttt{ here.}}
\end{DoxyParagraph}
...
\end{DoxyParagraph}
\hypertarget{md_pages_demos_demo3_autotoc_md19}{}\doxysection{Overview}\label{md_pages_demos_demo3_autotoc_md19}
This demo solves the same problem as the one \href{https://pressio.github.io/pressio4py/html/md_pages_demos_demo2.html}{\texttt{ here}}, but instead of using P\+OD modes, we show here how to use a nonlinear manifold computed via kernel P\+CA.\hypertarget{md_pages_demos_demo3_autotoc_md20}{}\doxysection{Imports}\label{md_pages_demos_demo3_autotoc_md20}
...
\end{DoxyCode}
\hypertarget{md_pages_demos_demo3_autotoc_md21}{}\doxysection{Main function}\label{md_pages_demos_demo3_autotoc_md21}
...
\DoxyCodeLine{  ax.plot(fomObj.xGrid, approximatedState, \textcolor{stringliteral}{'or'}, label=\textcolor{stringliteral}{'LSPG: with kPCA mapping'})}
...
\end{DoxyCode}
\hypertarget{md_pages_demos_demo3_autotoc_md22}{}\doxysection{Code for the various stages in main}\label{md_pages_demos_demo3_autotoc_md22}
Here we list the functions performing the various stages of the run.\hypertarget{md_pages_demos_demo3_autotoc_md23}{}\doxysubsection{1. Run F\+O\+M and collect snapshots}\label{md_pages_demos_demo3_autotoc_md23}
This step is the same as described \href{https://pressio.github.io/pressio4py/html/md_pages_demos_demo2.html}{\texttt{ here}},\hypertarget{md_pages_demos_demo3_autotoc_md24}{}\doxysubsection{2. Setup and train the nonlinear k\+P\+C\+A mapper}\label{md_pages_demos_demo3_autotoc_md24}
It is important to note that while the mapper class below has the A\+PI required by pressio4py, it can encapsulate any arbitrary mapping function. In this case we show how to create a k\+P\+C\+A-\/based representation, but one can use, e.\+g., autoencoder, and any other types of mapping. This is how we enable support for testing various methods. 
\begin{DoxyCode}{0}
...
\DoxyCodeLine{    \textcolor{keywordflow}{return} np.squeeze(self.transformer\_.transform(fomState.reshape(1,-\/1)))}
\DoxyCodeLine{}
...
when creating an abitrary mapping (as in the class above), the jacobian matrix {\bfseries{must}} be column-\/major oder so that pressio can reference it without deep copying it. This not only reduced the memory footprint since it allows to keep only one jacobian object around but also it is fundamental for the update method below correctly.
\end{DoxyParagraph}
\hypertarget{md_pages_demos_demo3_autotoc_md25}{}\doxysubsection{3. Construct and run L\+S\+PG}\label{md_pages_demos_demo3_autotoc_md25}
