...
Suppose now that an application involves $m$ fields, e.\+g., density, x-\/velocity, tracer concentration, etc, and rather than storing all degrees of freedom in a single array, one wants to keep them separate.

In such case, the F\+OM state can be represented as a matrix, $y_{fom} \in R^{N,m}$, where\+:
\begin{DoxyItemize}
...
This scenario is intended for applications that prefer to separate the degress of freedom, and is frequently found when using explicit time integration.
