...
Note that the above expressions are abtract, since they do not specify what kind of data structure is used for $y_{fom}$. We envision the following scenarios\+:
\begin{DoxyItemize}
\item rank-\/1 state\+: the F\+OM state is stored as an array, $y_{fom} \in R^N$, where $N$ = {\bfseries{total number of degrees of freedom}}. In this case, the application possibly involves multiple fields (e.\+g., density, chemical species, etc), but stores in a single array all the spatial degrees of freedom. this is common when the application needs to do implicit time-\/integration such that a large system needed to be solved.
