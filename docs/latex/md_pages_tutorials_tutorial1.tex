...
A key assumption of projection-\/based R\+O\+Ms relies on approximating a full-\/order model (F\+OM) state, $fomState$, as\+: \[ fomState = g(romState) \]

where $romState$ is the reduced state, also called generalized coordinates, and $g$ is the mapping between the two. If $g$ is linear, then we can write\+: \[ fomState = \phi romState \] where $\phi$ is a matrix (for the time being, assume it constant). Due to the linearity, the Jacobian of the mapping is\+: \[ \frac{d fomState}{d romState} = \phi. \]
