...
\end{DoxyParagraph}
\hypertarget{md_pages_tutorials_tutorial1_autotoc_md46}{}\doxysection{Context}\label{md_pages_tutorials_tutorial1_autotoc_md46}
...
where $y_{rom}$ is the reduced state, also called generalized coordinates, and $g$ is the mapping between the two. If $g$ is linear, then we can write\+: \[ y_{fom} = \phi y_{rom} \] where $\phi$ is a matrix (for the time being, assume it constant). A linear decoder in pressio4py implements this linear mapping. Due to the linearity, the Jacobian of the mapping is\+: \[ \frac{d y_{fom}}{d y_{rom}} = \phi. \]\hypertarget{md_pages_tutorials_tutorial1_autotoc_md47}{}\doxysection{Code}\label{md_pages_tutorials_tutorial1_autotoc_md47}
...
\DoxyCodeLine{  \textcolor{comment}{\#}}
\DoxyCodeLine{  \textcolor{comment}{\# Note: you can also use a row-\/major layout, but that forces}}
\DoxyCodeLine{  \textcolor{comment}{\# pressio4py to make a deep copy of matrix.}}
\DoxyCodeLine{}
