...
To address these questions, pressio4py provides a variant of the Galerkin problem, called {\itshape masked Galerkin}, that allows one to test the {\itshape accuracy} of various hyper-\/reduction techniques {\bfseries{without}} having to change the F\+OM code.



\begin{DoxyParagraph}{}
This tutorial shows how to construct a masked Galerkin problem using the most basic hyper-\/reduction technique, namely {\itshape collocation}.
\end{DoxyParagraph}
...
In practice, to use a {\itshape masked} Galerkin problem one needs\+:
\begin{DoxyEnumerate}
...
\item a masker object\+: the role of the ma 
\end{DoxyEnumerate}