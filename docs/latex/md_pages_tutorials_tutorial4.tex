...
\begin{DoxyEnumerate}
\item to create the decoder on the {\bfseries{F\+U\+LL}} mesh 
\begin{DoxyCode}{0}
...
\item to select the row indices to use for the collocation 
\begin{DoxyCode}{0}
...
\item to create a \char`\"{}projector operator\char`\"{} that is responsible to project the F\+OM velocity. Basically, this projector knows how to compute the action of $(A\phi)^T$. 
\begin{DoxyCode}{0}
...
\item creating the actual masked Galerkin problem\+: note that as in the default case, the time-\/stepping scheme is part of the class name. 
\begin{DoxyCode}{0}
\DoxyCodeLine{problem = rom.galerkin.masked.ProblemForwardEuler(fomObj, linearDecoder, romState, fomReferenceState, masker, projector)}
\end{DoxyCode}

\end{DoxyEnumerate}\hypertarget{md_pages_tutorials_tutorial4_autotoc_md78}{}\doxysection{How to solve a default Galerkin problem?}\label{md_pages_tutorials_tutorial4_autotoc_md78}
